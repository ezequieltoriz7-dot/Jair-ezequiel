...
const APP_STORAGE_KEY = 'umbral_analitica_v2_final';
const LEGACY_KEYS = ['umbral_analitica_v2', 'umbral_analitica_pro', 'choir_sync_data']; 
...
const Persistence = {
  ...
  load: <T,>(key: string, defaultValue: T): T => {
    try {
      ...
      if (saved) {
        ...
        if (key === 'choirs' && Array.isArray(parsed)) {
          return (defaultValue as any).map((defItem: any) => {
  ...
  exportData: () => {
    const data = {
      ...
      members: Persistence.load('members', INITIAL_MEMBERS),
      reports: Persistence.load('reports', []),
      events: Persistence.load('events', INITIAL_EVENTS)
    };
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleSync = () => {
      setAllUsers(Persistence.load('users', allUsers));
      setChoirs(Persistence.load('choirs', choirs));
      setMembers(Persistence.load('members', members));
      setReports(Persistence.load('reports', reports));
      setEvents(Persistence.load('events', events));
    };
  ...
  const handleLogout = () => { setUser(null); setAdminChoirFilter(null); setCurrentView(ViewType.DASHBOARD); setIsProfileModalOpen(false); };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f8fafc] dark:bg-midnight font-sans transition-colors duration-700">
      ...
      <main className="flex-1 overflow-y-auto relative bg-[#f8fafc] dark:bg-black/10">
        <div className="lg:hidden p-4 bg-white dark:bg-card-dark border-b border-slate-200 dark:border-white/5 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3">
             <div className="size-9 bg-primary rounded-xl flex items-center justify-center text-white shadow-lg">
                <span className="material-symbols-outlined text-xl">music_note</span>
             </div>
             <span className="font-black text-sm tracking-tighter text-slate-900 dark:text-white uppercase">EL UMBRAL</span>
          </div>
        ...
        {isProfileModalOpen && user && (
          <DirectorProfileModal 
            ...
            onExportData={Persistence.exportData}
            members={members}
            reports={reports}
            choirs={choirs}
          />
        ...
        <button onClick={() => setIsDarkMode(!isDarkMode)} className="fixed bottom-6 right-6 size-14 bg-secondary text-white dark:text-black rounded-2xl shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all border-4 border-white dark:border-card-dark z-50">
